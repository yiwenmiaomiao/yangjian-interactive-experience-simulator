...
## 交互铺垫规则（强制）
每个 beat 的 plot 必须为该 beat 的 transition goal 提供**足够的交互铺垫**，确保用户能自然推断出目标动作：

1. **如果 transition goal 要求物理交互（触碰/走近/拿起/打开等），plot 必须先描述该对象的物理特征，让用户知道它可以被交互**。
   - ✅ 正确示例："裂缝中飘出一段破碎的光影，你伸手触碰时，感受到一阵微凉——它们似乎有实体。"
   - ❌ 错误示例（m3 原本的问题）："裂缝中飘出一段破碎的记忆影像"，用户无法推断可以触碰。

2. **如果 transition goal 要求用户做出某种选择，plot 必须提供足够的背景让用户理解为什么要做这个选择**。
   - 正确示例：杨戬说"你可以选择离开，或者留下面对"，用户知道两个选项的后果。

3. **plot 禁止以内心活动或纯感受开头**，必须先建立外部可感知的环境/事件，在此基础上才允许内心活动。
